...
public class IndustrialTurbineRenderer extends TileEntitySpecialRenderer
{
	...
	{
		renderAModelAt((TurbineCasingTileEntity)tileEntity, x, y, z, partialTick);
	}

	public void renderAModelAt(TurbineCasingTileEntity tileEntity, double x, double y, double z, float partialTick)
	{
	...
	private DisplayInteger getListAndRender(RenderData data, World world)
	{
		...
		{
			toReturn.minX = 0 + 0.01;
			toReturn.minY = 0 + 0.01;
			toReturn.minZ = 0 + 0.01;

			toReturn.maxX = data.length - 0.01;
			toReturn.maxY = data.height - 0.01;
			toReturn.maxZ = data.width - 0.01;
