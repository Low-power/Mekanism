...
public class Reactor extends BlockContainer implements IBlockCTM
{
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int facing, float playerX, float playerY, float playerZ)
	{
		...
		{
			if(MekanismUtils.isBCWrench(player.getCurrentEquippedItem().getItem()) && !player.getCurrentEquippedItem().getUnlocalizedName().contains("omniwrench"))
			{
		...
		if(tileEntity instanceof TileEntityReactorController)
		{
			if(!player.isSneaking())
			{
				player.openGui(MekanismGenerators.instance, ReactorBlockType.get(this, metadata).guiId, world, x, y, z);
				return true;
			}
		}
