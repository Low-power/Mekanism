...
public class DigitalMinerConfigGui extends GuiMekanism
{
	...
	protected void drawGuiContainerBackgroundLayer(float partialTick, int mouseX, int mouseY)
	{
		...
		mc.renderEngine.bindTexture(MekanismUtils.getResource(ResourceType.GUI, "DigitalMinerConfigGui.png"));
		...
		for(int i = 0; i < 4; i++)
		{
			if(tileEntity.filters.get(getFilterIndex()+i) != null)
			{
				...
				if(filter instanceof MItemStackFilter)
				{
					MekanismRenderer.color(EnumColor.INDIGO, 1F, 2.5F);
				}
				else if(filter instanceof MOreDictFilter)
				{
					MekanismRenderer.color(EnumColor.BRIGHT_GREEN, 1F, 2.5F);
				}
				else if(filter instanceof MMaterialFilter)
				{
					MekanismRenderer.color(EnumColor.PURPLE, 1F, 4F);
				}
				else if(filter instanceof MModIDFilter)
				{
					MekanismRenderer.color(EnumColor.PINK, 1F, 2.5F);
				}
