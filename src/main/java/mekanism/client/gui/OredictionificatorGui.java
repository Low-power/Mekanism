...
public class OredictionificatorGui extends GuiMekanism
{
	...
	public OredictionificatorGui(InventoryPlayer inventory, TileEntityOredictionificator tentity)
	{
		...
		}, ProgressBar.LARGE_RIGHT, this, MekanismUtils.getResource(ResourceType.GUI, "OredictionificatorGui.png"), 62, 118));
		guiElements.add(new GuiSlot(SlotType.NORMAL, this, MekanismUtils.getResource(ResourceType.GUI, "OredictionificatorGui.png"), 25, 114));
		guiElements.add(new GuiSlot(SlotType.NORMAL, this, MekanismUtils.getResource(ResourceType.GUI, "OredictionificatorGui.png"), 133, 114));
		ySize+=64;
	}
	...
	protected void actionPerformed(GuiButton guibutton)
	{
		...
		if(guibutton.id == 0)
		{
			Mekanism.packetHandler.sendToServer(new OredictionificatorGuiMessage(OredictionificatorGuiPacketType.SERVER, Coord4D.get(tileEntity), 1, 0, 0));
		}
	}
	...
	{
		mc.renderEngine.bindTexture(MekanismUtils.getResource(ResourceType.GUI, "OredictionificatorGui.png"));
		...
		for(int i = 0; i < 3; i++)
		{
			if(tileEntity.filters.get(getFilterIndex()+i) != null)
			{
				...
				if(mouseOver)
				{
					MekanismRenderer.color(EnumColor.GREY, 3F);
				}
			...
			for(int i = 0; i < 3; i++)
			{
				if(tileEntity.filters.get(getFilterIndex()+i) != null)
				{
					...
					if(xAxis > 10 && xAxis <= 152 && yAxis > yStart && yAxis <= yStart+22)
					{
						...
						Mekanism.packetHandler.sendToServer(new OredictionificatorGuiMessage(OredictionificatorGuiPacketType.SERVER_INDEX, Coord4D.get(tileEntity), 1, getFilterIndex()+i, 0));
					}
				}
			}
		}
