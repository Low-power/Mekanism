...
public class ConfigurableMachineRenderer extends TileEntitySpecialRenderer
{
	...
	public ConfigurableMachineRenderer()
	{
	...
	public void renderAModelAt(ISideConfiguration configurable, double x, double y, double z, float partialTick)
	{
		...
		if(pos != null && itemStack != null) {
			...
			if(item instanceof Configurator && ((Configurator)item).getState(itemStack).isConfigurating()) {
				...
				TransmissionType type = ((Configurator)item).getState(itemStack).getTransmission();
				if(configurable.getConfig().supports(type))
				{
					if(xPos == tileEntity.xCoord && yPos == tileEntity.yCoord && zPos == tileEntity.zCoord)
					{
						...
						if(data != ConfigTileComponent.EMPTY)
						{
							push();
