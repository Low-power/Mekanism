...
public class TransmitterPartRenderer implements IIconSelfRegister
{
	public static TransmitterPartRenderer INSTANCE;
	...
	{
		INSTANCE = new TransmitterPartRenderer();
	...
	public void renderContents(PartLogisticalTransporter transporter, float partialTick, Vector3 vec)
	{
		...
		item_entity.age = 0;
		item_entity.hoverStart = 0;

		item_entity.setPosition(transporter.x() + 0.5, transporter.y() + 0.5, transporter.z() + 0.5);
		item_entity.worldObj = transporter.world();
		...
		for(TransporterStack stack : transporter.getTransmitter().transit)
		{
			...
			{
				GL11.glPushMatrix();
				item_entity.setEntityItemStack(stack.itemStack);

				float[] pos = TransporterUtils.getStackPosition(transporter.getTransmitter(), stack, partialTick*transporter.tier.speed);

				GL11.glTranslated(vec.x + pos[0], vec.y + pos[1] - item_entity.yOffset, vec.z + pos[2]);
				GL11.glScalef(0.75F, 0.75F, 0.75F);

				renderer.doRender(item_entity, 0, 0, 0, 0, 0);
				GL11.glPopMatrix();
				...
				if(stack.color != null)
				{
					...
					GL11.glColor4f(stack.color.getColor(0), stack.color.getColor(1), stack.color.getColor(2), 1F);
					GL11.glTranslatef((float)(vec.x + pos[0]), (float)(vec.y + pos[1] - item_entity.yOffset - ((stack.itemStack.getItem() instanceof ItemBlock) ? 0.1 : 0)), (float)(vec.z + pos[2]));
		...
		if(transporter instanceof PartDiversionTransporter)
		{
			...
			MovingObjectPosition pos = player.rayTrace(8D, 1F);

			if(pos != null && itemStack != null && itemStack.getItem() instanceof Configurator)
			{
