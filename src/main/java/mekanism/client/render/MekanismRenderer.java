...
public class MekanismRenderer {
	...
	public static ConfigurableMachineRenderer machineRenderer = new ConfigurableMachineRenderer();
	...
	public void onStitch(TextureStitchEvent.Pre event)
	{
		if(event.map.getTextureType() == 0)
		{
			...
			if(TransmitterPartRenderer.getInstance() != null)
			{
				TransmitterPartRenderer.getInstance().resetDisplayInts();
			}
			RenderDynamicTank.resetDisplayInts();
			RenderThermalEvaporationController.resetDisplayInts();
			FluidTankRenderer.resetDisplayInts();
	...
	public static void prepareItemRender(RenderBlocks renderer, int metadata, Block block) {
		...
		if(renderer.useInventoryTint) {
			...
			float red = (renderColor >> 16 & 255) / 255F;
			float green = (renderColor >> 8 & 255) / 255F;
			float blue = (renderColor & 255) / 255F;
	...
	public static void renderCustomItem(RenderBlocks renderer, ItemStack stack) {
		...
		if(block instanceof ICustomBlockIcon) {
			...
			try {
				...
				renderer.renderFaceYNeg(block, 0D, 0D, 0D, custom.getIcon(stack, 0));
				tessellator.draw();
				tessellator.startDrawingQuads();
				tessellator.setNormal(0F, 1F, 0F);
				renderer.renderFaceYPos(block, 0D, 0D, 0D, custom.getIcon(stack, 1));
				tessellator.draw();
				tessellator.startDrawingQuads();
				tessellator.setNormal(0F, 0F, -1F);
				renderer.renderFaceZNeg(block, 0D, 0D, 0D, custom.getIcon(stack, 3));
				tessellator.draw();
				tessellator.startDrawingQuads();
				tessellator.setNormal(0F, 0F, 1F);
				renderer.renderFaceZPos(block, 0D, 0D, 0D, custom.getIcon(stack, 2));
				tessellator.draw();
				tessellator.startDrawingQuads();
				tessellator.setNormal(-1F, 0F, 0F);
				renderer.renderFaceXNeg(block, 0D, 0D, 0D, custom.getIcon(stack, 4));
				tessellator.draw();
				tessellator.startDrawingQuads();
				tessellator.setNormal(1F, 0F, 0F);
				renderer.renderFaceXPos(block, 0D, 0D, 0D, custom.getIcon(stack, 5));
				tessellator.draw();
			} catch(Exception e) {
				e.printStackTrace();
			}
	...
	public static void renderItem(RenderBlocks renderer, int metadata, Block block)
	{
		...
		try {
			...
			tessellator.startDrawingQuads();
			tessellator.setNormal(0F, 1F, 0F);
			renderer.renderFaceYPos(block, 0D, 0D, 0D, block.getIcon(1, metadata));
			tessellator.draw();
			tessellator.startDrawingQuads();
			tessellator.setNormal(0F, 0F, -1F);
			renderer.renderFaceZNeg(block, 0D, 0D, 0D, block.getIcon(3, metadata));
			tessellator.draw();
			tessellator.startDrawingQuads();
			tessellator.setNormal(0F, 0F, 1F);
			renderer.renderFaceZPos(block, 0D, 0D, 0D, block.getIcon(2, metadata));
			tessellator.draw();
			tessellator.startDrawingQuads();
			tessellator.setNormal(-1F, 0F, 0F);
			renderer.renderFaceXNeg(block, 0D, 0D, 0D, block.getIcon(4, metadata));
			tessellator.draw();
			tessellator.startDrawingQuads();
			tessellator.setNormal(1F, 0F, 0F);
			renderer.renderFaceXPos(block, 0D, 0D, 0D, block.getIcon(5, metadata));
			tessellator.draw();
		} catch(Exception e) {
			e.printStackTrace();
		}
