...
import mekanism.common.RobitFollowTask;
import mekanism.common.RobitPickupTask;
...
public class Robit extends EntityCreature implements IInventory, ISustainedInventory, IEntityBreathable
{
	...
	public Robit(World world)
	{
		...
		tasks.addTask(1, new RobitPickupTask(this, 1F));
		tasks.addTask(2, new RobitFollowTask(this, 1F, 4F, 2F));
	...
	public Robit(World world, double x, double y, double z)
	{
		this(world);
	...
	public void onEntityUpdate()
	{
		...
		if(!worldObj.isRemote)
		{
			...
			if(!(homeLocation.getTileEntity(MinecraftServer.getServer().worldServerForDimension(homeLocation.dimensionId)) instanceof ChargepadTileEntity))
			{
				drop();
				setDead();
	...
	private void collectItems()
	{
		...
		if(items != null && !items.isEmpty())
		{
			...
			{
				if(item.delayBeforeCanPickup > 0 || item.getEntityItem().getItem() instanceof RobitItem)
				{
					continue;
				}
				...
				for(int i = 0; i < 27; i++)
				{
					...
					if(itemStack == null)
					{
						...
						playSound("random.pop", 1F, ((rand.nextFloat() - rand.nextFloat()) * 0.7F + 1F) * 2F);
	...
	public boolean isOnChargepad()
	{
		...
		if(worldObj.getTileEntity(x, y, z) instanceof ChargepadTileEntity)
		{
			return true;
		}
	...
	public boolean interact(EntityPlayer player)
	{
		if(player.isSneaking())
		{
			...
			player.openGui(Mekanism.instance, 21, worldObj, getEntityId(), 0, 0);
		}
	...
	public void drop()
	{
		EntityItem item_entity = new EntityItem(worldObj, posX, posY+0.3, posZ, new ItemStack(MekanismItems.Robit));

		RobitItem item = (RobitItem)item_entity.getEntityItem().getItem();
		item.setEnergy(item_entity.getEntityItem(), getEnergy());
		item.setInventory(getInventory(), item_entity.getEntityItem());
		item.setName(item_entity.getEntityItem(), getCommandSenderName());

		float k = 0.05F;
		item_entity.motionX = 0;
		item_entity.motionY = rand.nextGaussian() * k + 0.2F;
		item_entity.motionZ = 0;

		worldObj.spawnEntityInWorld(item_entity);
	}
