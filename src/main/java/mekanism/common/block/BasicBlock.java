 ...
 *
 */
public class BasicBlock extends Block implements IBlockCTM, ICustomBlockIcon
{
	...
	public IIcon getIcon(ItemStack stack, int side)
	{
		...
		{
			return binIcons[((BasicBlockItem)stack.getItem()).getBaseTier(stack).ordinal()][side];
		}
		else if(BasicType.get(stack) == BasicType.INDUCTION_CELL)
		{
			return icons[3][((BasicBlockItem)stack.getItem()).getBaseTier(stack).ordinal()];
		}
		else if(BasicType.get(stack) == BasicType.INDUCTION_PROVIDER)
		{
			return icons[4][((BasicBlockItem)stack.getItem()).getBaseTier(stack).ordinal()];
		}
	...
	public IIcon getIcon(IBlockAccess world, int x, int y, int z, int side)
	{
		...
		switch(blockType)
		{
			case BASIC_BLOCK_1:
				...
				{
					case 6:
						BinTileEntity tileEntity = (BinTileEntity)world.getTileEntity(x, y, z);
	...
	public IIcon getIcon(int side, int meta)
	{
		...
		{
			case BASIC_BLOCK_1:
				return icons[meta][meta == 14 && side != 2 ? 2 : 0];
			case BASIC_BLOCK_2:
				return icons[meta][0];
			default:
				return icons[meta][0];
					...
					case BIN:
						for(BaseTier tier : BaseTier.values())
						{
							if(tier.isObtainable())
							{
								...
								((BasicBlockItem)stack.getItem()).setBaseTier(stack, tier);
								list.add(stack);
							}
						}
						break;
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int i1, float f1, float f2, float f3)
	{
		...
		if(type == BasicType.REFINED_OBSIDIAN)
		{
			if(player.isSneaking())
			{
				player.openGui(Mekanism.instance, 1, world, x, y, z);
				return true;
			}
		}
		...
		if(tile instanceof TileEntityThermalEvaporationController)
		{
			if(!player.isSneaking())
			{
				if(!world.isRemote)
				{
					player.openGui(Mekanism.instance, 33, world, x, y, z);
				}
				return true;
			}
		}
		else if(tile instanceof BinTileEntity)
		{
			...
			if(player.getCurrentEquippedItem() != null && MekanismUtils.hasUsableWrench(player, x, y, z))
			{
				if(!world.isRemote)
				{
			...
			if(!world.isRemote)
			{
				...
				{
					if(bin.addTicks == 0 && player.getCurrentEquippedItem() != null)
					{
						if(player.getCurrentEquippedItem() != null)
						{
							ItemStack remain = bin.add(player.getCurrentEquippedItem());
							player.setCurrentItemOrArmor(0, remain);
							bin.addTicks = 5;
						}
					}
		...
		else if(tile instanceof IStructuralMultiblock)
		{
			...
			return ((IStructuralMultiblock)world.getTileEntity(x, y, z)).onActivate(player);
		}
	...
	public int getLightValue(IBlockAccess world, int x, int y, int z)
	{
		...
		if(blockType == BasicBlockType.BASIC_BLOCK_1)
		{
			switch(metadata)
			{
				case 2:
		...
		else if(blockType == BasicBlockType.BASIC_BLOCK_2)
		{
			if(metadata == 5)
			{
	...
	{
		if(world.getTileEntity(x, y, z) instanceof BasicBlockTileEntity)
		{
			BasicBlockTileEntity tileEntity = (BasicBlockTileEntity)world.getTileEntity(x, y, z);
			int side = MathHelper.floor_double((entity.rotationYaw * 4F / 360F) + 0.5D) & 3;
	...
	public ItemStack getPickBlock(MovingObjectPosition target, World world, int x, int y, int z, EntityPlayer player)
	{
		...
		else if(type == BasicType.INDUCTION_CELL)
		{
			...
			((BasicBlockItem)ret.getItem()).setBaseTier(ret, tileEntity.tier.getBaseTier());
		}
	...
	{
		OSMIUM_BLOCK(BasicBlockType.BASIC_BLOCK_1, 0, "OsmiumBlock", null, false),
		BRONZE_BLOCK(BasicBlockType.BASIC_BLOCK_1, 1, "BronzeBlock", null, false),
		REFINED_OBSIDIAN(BasicBlockType.BASIC_BLOCK_1, 2, "RefinedObsidian", null, false),
		CHARCOAL_BLOCK(BasicBlockType.BASIC_BLOCK_1, 3, "CharcoalBlock", null, false),
		REFINED_GLOWSTONE(BasicBlockType.BASIC_BLOCK_1, 4, "RefinedGlowstone", null, false),
		STEEL_BLOCK(BasicBlockType.BASIC_BLOCK_1, 5, "SteelBlock", null, false),
		BIN(BasicBlockType.BASIC_BLOCK_1, 6, "Bin", BinTileEntity.class, true),
		TELEPORTER_FRAME(BasicBlockType.BASIC_BLOCK_1, 7, "TeleporterFrame", null, true),
		STEEL_CASING(BasicBlockType.BASIC_BLOCK_1, 8, "SteelCasing", null, true),
		DYNAMIC_TANK(BasicBlockType.BASIC_BLOCK_1, 9, "DynamicTank", TileEntityDynamicTank.class, true),
		STRUCTURAL_GLASS(BasicBlockType.BASIC_BLOCK_1, 10, "StructuralGlass", TileEntityStructuralGlass.class, true),
		DYNAMIC_VALVE(BasicBlockType.BASIC_BLOCK_1, 11, "DynamicValve", TileEntityDynamicValve.class, true),
		COPPER_BLOCK(BasicBlockType.BASIC_BLOCK_1, 12, "CopperBlock", null, false),
		TIN_BLOCK(BasicBlockType.BASIC_BLOCK_1, 13, "TinBlock", null, false),
		THERMAL_EVAPORATION_CONTROLLER(BasicBlockType.BASIC_BLOCK_1, 14, "ThermalEvaporationController", TileEntityThermalEvaporationController.class, true),
		THERMAL_EVAPORATION_VALVE(BasicBlockType.BASIC_BLOCK_1, 15, "ThermalEvaporationValve", TileEntityThermalEvaporationValve.class, true),
		THERMAL_EVAPORATION_BLOCK(BasicBlockType.BASIC_BLOCK_2, 0, "ThermalEvaporationBlock", TileEntityThermalEvaporationBlock.class, true),
		INDUCTION_CASING(BasicBlockType.BASIC_BLOCK_2, 1, "InductionCasing", TileEntityInductionCasing.class, true),
		INDUCTION_PORT(BasicBlockType.BASIC_BLOCK_2, 2, "InductionPort", TileEntityInductionPort.class, true),
		INDUCTION_CELL(BasicBlockType.BASIC_BLOCK_2, 3, "InductionCell", TileEntityInductionCell.class, true),
		INDUCTION_PROVIDER(BasicBlockType.BASIC_BLOCK_2, 4, "InductionProvider", TileEntityInductionProvider.class, true),
		...
		private BasicType(BasicBlockType block_type, int i, String s, Class<? extends TileEntity> tileClass, boolean hasDesc)
		{
		...
		public static BasicType get(Block block, int meta)
		{
			...
			{
				return get(((BasicBlock)block).blockType, meta);
			}

			return null;
		}

		public static BasicType get(BasicBlockType block, int meta)
		{
			for(BasicType type : values())
			{
				if(type.meta == meta && type.block_type == block)
				{
					return type;
				}
			}
		...
		{
			return new ItemStack(block_type.getBlock(), 1, meta);
		}
