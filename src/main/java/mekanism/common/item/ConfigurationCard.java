...
public class ConfigurationCard extends ItemMekanism
{
	...
	private String getNameFromTile(TileEntity tile)
	{
		...
		{
			return ((ISpecialConfigData)tile).getDataType();
		}
		if(tile instanceof ContainerTileEntity)
		{
			return tile.getBlockType().getUnlocalizedName() + "." + ((ContainerTileEntity)tile).fullName + ".name";
		}
		return Integer.toString(tile.hashCode());
	}
