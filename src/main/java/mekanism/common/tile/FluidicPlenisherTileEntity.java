...
public class FluidicPlenisherTileEntity extends TileEntityElectricBlock implements IComputerIntegration, IConfigurable, IFluidHandler, ISustainedTank, IUpgradeTile, IRedstoneControl
{
	...
	public FluidicPlenisherTileEntity()
	{
	...
	public void onUpdate()
	{
		if(!worldObj.isRemote)
		{
			...
			if(MekanismUtils.canFunction(this) && getEnergy() >= energyPerTick && fluid_stack != null && fluid_stack.getFluid().canBePlacedInWorld())
			{
				if(!finishedCalc)
				{
				...
				if((operatingTicks + 1) < ticksRequired)
				{
					...
					if(!finishedCalc)
					{
						...
						{
							if(fluid_stack.getFluid().canBePlacedInWorld())
							{
								worldObj.setBlock(below.xCoord, below.yCoord, below.zCoord, MekanismUtils.getFlowingBlock(fluid_stack.getFluid()), 0, 3);
								setEnergy(getEnergy() - energyPerTick);
	...
	public ArrayList getNetworkedData(ArrayList data)
	{
		...
		data.add(Boolean.valueOf(finishedCalc));
