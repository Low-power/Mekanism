...
public class ElectrolyticSeparatorTileEntity extends TileEntityElectricBlock implements IFluidHandler, IComputerIntegration, ITubeConnection, ISustainedData, IGasHandler, IUpgradeTile, IUpgradeInfoHandler, ITankManager, IRedstoneControl, IActiveState
{
	...
	public ElectrolyticSeparatorTileEntity()
	{
	...
	public void onUpdate()
	{
		...
		if(!worldObj.isRemote)
		{
			...
			if(inventory[0] != null)
			{
				if(RecipeHandler.Recipe.ELECTROLYTIC_SEPARATOR.containsRecipe(inventory[0]))
				{
					if(inventory[0].getItem() instanceof IFluidContainerItem)
					{
						...
						FluidStack item_fluid = FluidContainerRegistry.getFluidForFilledItem(inventory[0]);
						FluidStack tank_fluid = fluidTank.getFluid();
						if(item_fluid != null && (tank_fluid == null || item_fluid.isFluidEqual(tank_fluid) && tank_fluid.amount + item_fluid.amount <= fluidTank.getCapacity()))
						{
							fluidTank.fill(item_fluid, true);
			...
			if(canOperate(recipe) && getEnergy() >= energyPerTick && MekanismUtils.canFunction(this))
			{
				...
				if(prevEnergy >= getEnergy()) setActive(false);
			}
	...
	public ArrayList getNetworkedData(ArrayList data)
	{
		...
		data.add(Integer.valueOf(dumpLeft.ordinal()));
		data.add(Integer.valueOf(dumpRight.ordinal()));
	...
	public Object[] invoke(int method, Object[] arguments) throws Exception
	{
		...
		switch(method)
		{
			...
			case 4:
				fluid_stack = fluidTank.getFluid();
				return new Object[] {fluid_stack != null ? fluid_stack.amount : 0};
			case 5:
				fluid_stack = fluidTank.getFluid();
				return new Object[] {fluid_stack != null ? (fluidTank.getCapacity() - fluid_stack.amount) : 0};
			case 6:
	...
	public void writeSustainedData(ItemStack itemStack) {
		...
		if(gas_stack != null)
		{
			itemStack.stackTagCompound.setTag("leftTank", gas_stack.write(new NBTTagCompound()));
		}
		gas_stack = rightTank.getGas();
		if(gas_stack != null)
		{
			itemStack.stackTagCompound.setTag("rightTank", gas_stack.write(new NBTTagCompound()));
		}
	}
