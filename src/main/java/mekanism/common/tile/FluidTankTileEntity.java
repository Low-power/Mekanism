...
public class FluidTankTileEntity extends ContainerTileEntity implements IActiveState, IConfigurable, IFluidHandler, ISustainedTank, IFluidContainerManager, ITankManager, ITierUpgradeable
{
	...
	public void onUpdate() {
		if(worldObj.isRemote)
		{
			...
			float targetScale = (float)(fluid_stack != null ? fluid_stack.amount : 0) / fluidTank.getCapacity();
	...
	private void activeEmit()
	{
		...
		if(fluid_stack != null)
		{
			...
			{
				FluidStack toDrain = new FluidStack(fluid_stack, Math.min(tier.output, fluidTank.getFluidAmount()));
	...
	public int pushUp(FluidStack fluid, boolean doFill)
	{
		...
		if(up.getTileEntity(worldObj) instanceof FluidTankTileEntity)
		{
	...
	public boolean isItemValidForSlot(int slotID, ItemStack itemstack)
	{
		if(slotID == 0)
		{
			...
			{
				FluidStack item_fluid_stack = FluidContainerRegistry.getFluidForFilledItem(itemstack);
				FluidStack tank_fluid_stack = fluidTank.getFluid();
				if(tank_fluid_stack == null || tank_fluid_stack.isFluidEqual(item_fluid_stack))
				{
	...
	public int getCurrentNeeded()
	{
		...
		if(topTile instanceof FluidTankTileEntity)
		{
			FluidTankTileEntity topTank = (FluidTankTileEntity)topTile;
			FluidStack top_tank_fluid = topTank.fluidTank.getFluid();
			FluidStack tank_fluid = fluidTank.getFluid();
			if(tank_fluid != null && top_tank_fluid != null)
			{
				if(tank_fluid.getFluid() != top_tank_fluid.getFluid())
				{
					return needed;
				}
			}
	...
	public ArrayList getNetworkedData(ArrayList data)
	{
		...
		data.add(Integer.valueOf(tier.ordinal()));
		data.add(Boolean.valueOf(isActive));
		data.add(Integer.valueOf(valve));
		data.add(Integer.valueOf(editMode.ordinal()));
		if(valve > 0)
		{
			data.add(Integer.valueOf(valveFluid.getID()));
		}
		FluidStack fluid_stack = fluidTank.getFluid();
		if(fluid_stack != null)
		{
	...
	public boolean canFill(ForgeDirection from, Fluid fluid) {
		if(from == ForgeDirection.DOWN)
		{
			...
			if(isActive && !(tile instanceof FluidTankTileEntity))
			{
				return false;
			}
		}
		FluidStack fluid_stack = fluidTank.getFluid();
		return fluid_stack == null || fluid_stack.getFluid() == fluid;
	}
	...
	public boolean canDrain(ForgeDirection from, Fluid fluid)
	{
		if(fluidTank != null)
		{
			...
			if(fluid == null || ((fluid_stack = fluidTank.getFluid()) != null && fluid_stack.getFluid() == fluid)) {
				if(isActive)
				{
