...
import mekanism.common.content.miner.MinerSearchThread;
...
public class DigitalMinerTileEntity extends TileEntityElectricBlock implements IUpgradeTile, IRedstoneControl, IActiveState, ISustainedData, IAdvancedBoundingBlock
{
	...
	public MinerSearchThread searcher = new MinerSearchThread(this);
	...
	public void reset()
	{
		searcher = new MinerSearchThread(this);
		running = false;
		oresToMine.clear();
		replaceMap.clear();
		missingStack = null;
	...
	public ArrayList getNetworkedData(ArrayList data)
	{
		...
		data.add(Integer.valueOf(radius));
		data.add(Integer.valueOf(minY));
		data.add(Integer.valueOf(maxY));
		data.add(Boolean.valueOf(doEject));
		data.add(Boolean.valueOf(doPull));
		data.add(Boolean.valueOf(isActive));
		data.add(Boolean.valueOf(running));
		data.add(Boolean.valueOf(silkTouch));
		data.add(Integer.valueOf(numPowering));
		data.add(Integer.valueOf(searcher.state.ordinal()));
		if(searcher.state == State.SEARCHING)
		{
			data.add(Integer.valueOf(searcher.found));
		}
		else {
			data.add(Integer.valueOf(getSize()));
		}

		data.add(Integer.valueOf(controlType.ordinal()));
		data.add(Boolean.valueOf(inverse));
		if(missingStack != null)
		{
			data.add(Boolean.valueOf(true));
			data.add(Integer.valueOf(MekanismUtils.getID(missingStack)));
			data.add(Integer.valueOf(missingStack.getItemDamage()));
		}
		else {
