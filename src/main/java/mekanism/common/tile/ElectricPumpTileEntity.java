...
public class ElectricPumpTileEntity extends TileEntityElectricBlock implements IFluidHandler, ISustainedTank, IConfigurable, IRedstoneControl, IUpgradeTile, ITankManager, IComputerIntegration
{
	/** This pump's tank */
	...
	public void onUpdate()
	{
		if(!worldObj.isRemote)
		{
			...
			if(MekanismUtils.canFunction(this) && getEnergy() >= energyPerTick)
			{
				...
				if((operatingTicks + 1) < ticksRequired)
				{
					...
					if(fluid_stack == null || fluid_stack.amount + FluidContainerRegistry.BUCKET_VOLUME <= fluidTank.getCapacity())
					{
						if(!suck(true))
						{
		...
		if(!worldObj.isRemote && (fluid_stack = fluidTank.getFluid()) != null)
		{
			TileEntity tileEntity = Coord4D.get(this).getFromSide(ForgeDirection.UP).getTileEntity(worldObj);

			if(tileEntity instanceof IFluidHandler)
			{
				FluidStack toDrain = new FluidStack(fluid_stack, Math.min(256*(upgradeComponent.getUpgrades(Upgrade.SPEED)+1), fluidTank.getFluidAmount()));
	...
	public boolean suck(boolean take)
	{
		...
		for(ForgeDirection orientation : ForgeDirection.VALID_DIRECTIONS)
		{
			...
			FluidStack world_fluid = MekanismUtils.getFluid(worldObj, wrapper, hasFilter());
			FluidStack tank_fluid = fluidTank.getFluid();
			if(world_fluid != null && (activeType == null || world_fluid.getFluid() == activeType) && (tank_fluid == null || tank_fluid.isFluidEqual(world_fluid)))
			{
				if(take)
				{
					activeType = world_fluid.getFluid();
					recurringNodes.add(wrapper.clone());
					fluidTank.fill(world_fluid, true);
					if(shouldTake(world_fluid, wrapper))
					{
		...
		for(Coord4D wrapper : tempPumpList)
		{
			...
			for(ForgeDirection orientation : ForgeDirection.VALID_DIRECTIONS)
			{
				...
				{
					world_fluid = MekanismUtils.getFluid(worldObj, side, hasFilter());
					if(world_fluid != null && (activeType == null || world_fluid.getFluid() == activeType) && (tank_fluid == null || tank_fluid.isFluidEqual(world_fluid)))
					{
						if(take)
						{
							...
							fluidTank.fill(world_fluid, true);
							if(shouldTake(world_fluid, side))
							{
	...
	private boolean shouldTake(FluidStack fluid_stack, Coord4D coord)
	{
		Fluid fluid = fluid_stack.getFluid();
		if(fluid == FluidRegistry.WATER || fluid == FluidRegistry.getFluid("heavywater"))
		{
	...
	{
		FluidStack fluid_stack = fluidTank.getFluid();
		return fluid_stack != null ? fluid_stack.amount * i / 10000 : 0;
	}
	...
	{
		FluidStack fluid_stack = fluidTank.getFluid();
		if(fluid_stack != null && fluid_stack.getFluid() == resource.getFluid() && from == ForgeDirection.getOrientation(1))
		{
